...
//                    fprintf(f, "%d %d %d %p %p\n", (int)n, (int)(max - left), (int)bioread, enc_data, enc_len);
//                    for(int i = 0; i < bioread; i++) {
//                        fprintf(f, "%02x ", 0xffu & *(pkt + len + left + i));
//                    }
                    memcpy(enc_data + *enc_len, pkt + len + left, bioread);
                    *enc_len += bioread;
//                    fprintf(f,"\n\n");
//                    fclose(f);
                }
...
int ssl3_read_bytes(SSL *s, int type, int *recvd_type, unsigned char *buf,
                    size_t len, int peek, size_t *readbytes, char* enc_data, int* enc_len)
{
    int i, j, ret;
 ...
 start:
    ...
    do {
        ...
        if (num_recs == 0) {
            ret = ssl3_get_record(s, enc_data, enc_len);
            if (ret <= 0) {
                /* SSLfatal() already called if appropriate */
                return ret;
            }
