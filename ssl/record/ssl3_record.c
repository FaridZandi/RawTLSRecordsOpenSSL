...
int ssl3_get_record(SSL *s, char* enc_data, int* enc_len)
{
    int enc_err, rret;
    int i;
    size_t more, n;
    ...
    do {
        ...
        if ((RECORD_LAYER_get_rstate(&s->rlayer) != SSL_ST_READ_BODY) ||
            ...
            rret = ssl3_read_n(s, SSL3_RT_HEADER_LENGTH,
                               ...
                               num_recs == 0 ? 1 : 0, &n, enc_data, enc_len);
        ...
        if (more > 0) {
            ...
            rret = ssl3_read_n(s, more, more, 1, 0, &n, enc_data, enc_len);
            if (rret <= 0)
 ...
 again:
    ...
    if ((RECORD_LAYER_get_rstate(&s->rlayer) != SSL_ST_READ_BODY) ||
        ...
        rret = ssl3_read_n(s, DTLS1_RT_HEADER_LENGTH,
                           SSL3_BUFFER_get_len(&s->rlayer.rbuf), 0, 1, &n, NULL, NULL);
    ...
    if (rr->length >
        ...
        more = rr->length;
        rret = ssl3_read_n(s, more, more, 1, 1, &n, NULL, NULL);
        /* this packet contained a partial record, dump it */
        if (rret <= 0 || n != more) {
            if (ossl_statem_in_error(s)) {
